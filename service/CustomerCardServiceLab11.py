...
class CustomerCardServiceLab11:
    """
    ...
    def showCardClientDescOrd(self):
        ...
        final_list = my_sorted(list_of_filtered_cards,
                               key=lambda cardDD: max_per_id[cardDD.get_id_entity()],
                               reverse=True)
        list_of_discount = []
        for card in final_list:
            list_of_discount.append(max_per_id[card.get_id_entity()])
        list_of_card_and_discount = zip(final_list, list_of_discount)
        return list_of_card_and_discount
